...
async def get_daily_overview(date: str = Query(...), ws: str = Depends(get_workspace_id)):
    try:
    ...
    dragons = await db.dragons.find({"workspace_id": ws}).sort("created_at", 1).to_list(1000)
    times = await db.times.find({"workspace_id": ws}).to_list(1000)
    times_map = {str(t["_id"]): t["time"] for t in times}
    settings_doc = await db.app_settings.find_one({"workspace_id": ws})
    language = (settings_doc or {}).get("language", "en")
    name_field = "name_da" if language == "da" else "name_en"
    task_items = await db.task_items.find({"workspace_id": ws}).to_list(1000)
    items_map = {str(i["_id"]): (i.get(name_field) or i.get("name")) for i in task_items}
    completions = await db.completions.find({"date": date, "workspace_id": ws}).to_list(1000)
...
async def get_calendar_summary(
    ...
    dragons = await db.dragons.find({"workspace_id": ws}).to_list(1000)
    if dragon_id:
    ...
    async def get_slots(day_of_week: str, age_category: str, activity_state: str):
        ...
        if key not in slots_cache:
            query = {"workspace_id": ws, "age_category": age_category, "day_of_week": day_of_week}
            if activity_state == "brumation":
    ...
    for day_num in range(1, days_in_month + 1):
        ...
        completions = await db.completions.find({"date": date_str, "workspace_id": ws}).to_list(1000)
        completion_map = {(c["dragon_id"], c["schedule_slot_id"]): c["completed"] for c in completions}
...
async def toggle_completion(payload: CompletionToggle, ws: str = Depends(get_workspace_id)):
