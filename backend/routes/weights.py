...
async def create_weight_entry(dragon_id: str, payload: WeightEntryCreate):
    try:
        d_oid = to_object_id(dragon_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="Ugyldigt agame-id")
    dragon = await db.dragons.find_one({"_id": d_oid})
    if not dragon:
        raise HTTPException(status_code=404, detail="Agame ikke fundet")
    entry = WeightEntry(dragon_id=dragon_id, **payload.model_dump())
...
async def list_weight_entries(dragon_id: str):
    docs = await db.weight_entries.find({"dragon_id": dragon_id}).sort("date", 1).to_list(2000)
...
async def delete_weight_entry(entry_id: str):
    ...
    result = await db.weight_entries.delete_one({"_id": oid})
